...
export class RefreshTokenUseCase {
  ...
  async execute(
    ...
    if (!existingToken || existingToken.expiresAt < new Date()) {
      ...
      return left(
        new InvalidCredentialsError('Invalid or expired refresh token.'),
      )
    }
    ...
    if (createNewTokenResult.isLeft()) {
      ...
      console.error(
        'CRITICAL: Failed to save new refresh token during rotation:',
        createNewTokenResult.value,
      )
