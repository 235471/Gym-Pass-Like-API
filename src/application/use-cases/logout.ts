...
export class LogoutUseCase {
  ...
  async execute(data: LogoutUseCaseInputDTO): Promise<LogoutUseCaseResponse> {
    ...
    const deleteResult = await this.refreshTokenRepository.deleteByUserId(userId);

    if (deleteResult.isLeft()) {
      // Log the error but proceed to return success for logout regardless
      console.error(`Failed to delete refresh tokens during logout for user ${userId}:`, deleteResult.value);
